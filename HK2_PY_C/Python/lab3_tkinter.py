...
#вариант 4
#Перевод заданного числа из 10-я сс в 4-ю и обратно
...
def calc():
    if measure.get()==0:
        ...
        if measure.get() == 1:
            for x in innumEntry.get():
                if x not in l_10 and x != '-' and x != '.' :
                    box.showinfo('Error', 'Input error, Вводите число.')
                    return
            ...
            if '.' in innumEntry.get():
                ...
                for x in range(len(s[0])):
                    ...
                    p *= 4
                    if int(s[0][x]) >= 4:
                        box.showinfo('Error', 'Введите число в четверичной сс!')
                        return
...
def showinfo2():
    box.showinfo('Info', 'Перевод заданного числа из 10-й сс в 4-ю и обратно')
...
#Choice
measure=IntVar()
change1=Radiobutton(window,text='Перевод из 10-й сс в 4-ю',variable=measure,value=1)
change2=Radiobutton(window,text='Перевод из 4-й сс в 10-ю',variable=measure,value=2)
