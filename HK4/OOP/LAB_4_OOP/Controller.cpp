...
Controller::Controller(QWidget *parent) : QWidget(parent)
{
    ...
    for (size_t i = 0; i < FLOOR_MAX; i++)
    {
        ...
        QObject::connect(new_button.get(), SIGNAL(pressSignal(bool,int)), this, SLOT(newTarget(bool,int))); // tạo thêm hàng chờ nhận từ file button
    }

    QObject::connect(this, SIGNAL(reachFloorSignal()), this, SLOT(reachFloor())); // đến tầng chỉ định
}
...
void Controller::newTarget(bool got_new, int floor)
{
    ...
    if (got_new)
    {
        ...
        if (_direction == STAY)
        {
            emit reachFloorSignal(); // gửi signal khi đến tầng chỉ định
        }
        else
        {
            emit moveCabinSignal(); // di chuyển cabin , gửi tín hiệu đến file Lift
        }
    }
...
void Controller::reachFloor()
{
    if (_status == BUSY)
    {
        ...
        emit stopCabinSignal();     // sau khi đến tầng chỉ định thì dừng và mở cửa, gửi signal đến Lift
    }
}
