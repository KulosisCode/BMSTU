...
void Door::beginOpening()
{
    if (_status == CLOSED || _status == CLOSING)
    {
        ...
        if (temp == CLOSED)
        {
            this->_openTimer.start(DOOR_TIME); // gửi signal  _openTimer
        }
        else
        {
            auto delta_timer = this->_closeTimer.remainingTime();
            _closeTimer.stop();
            this->_closeTimer.start(DOOR_TIME - delta_timer); // gửi signal _closeTimer
        }
...
void Door::wasOpened()
{
    if (_status == OPENING)
    {
        ...
        this->_waitTimer.start(WAIT_TIME); // kết thúc sẽ gửi signal _waitTimer
    }
}
...
void Door::beginClosing()
{
    if (_status == OPENED)
    {
        ...
        _closeTimer.start(DOOR_TIME); // kết thúc sẽ gửi signal  _closeTimer
    }
}
...
void Door::wasClosed()
{
    if (_status == CLOSING)
    {
        ...
        emit doorClosed(); // sau khi đóng cửa thì gửi signal đến file Cabin
    }
}
