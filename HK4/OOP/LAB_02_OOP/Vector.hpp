...
template <typename Type>
bool Vector<Type>::equals(const Vector<Type>& vec) const
{
    int equal = 1;
...
template <typename Type>
bool Vector<Type>::notequals(const Vector<Type>& vec) const
{
    if (*this == vec)
        return false;
    else
...
template<typename Type>
ConstIterator<Type> Vector<Type>::cbegin()
{
...
template<typename Type>
ConstIterator<Type> Vector<Type>::cend()
{
    return ConstIterator<Type>(*this) + num_elem;
}

template <typename Type>
Type Vector<Type>::scalar_mult(const Vector<Type>& vec) const
{
    time_t t_time = time(NULL);
...
{
    return scalar_mult(vec);
}

template <typename Type>
Vector<Type> Vector<Type>::summ(const Vector<Type>& vec) const
{
    time_t t_time = time(NULL);
    if (num_elem < 0 || vec.num_elem < 0)
        throw emptyError(__FILE__, typeid(*this).name(), __LINE__, ctime(&t_time));
...
template <typename Type>
Vector<Type> Vector<Type>::diff(const Vector<Type>& vec) const
{
    time_t t_time = time(NULL);
    if (num_elem < 0 || vec.num_elem < 0)
        throw emptyError(__FILE__, typeid(*this).name(), __LINE__, ctime(&t_time));
