...
def find_slau_matrix_3d(dots, n):
    rn = 0
...
def d3():
    ...
    slau_matrix = find_slau_matrix_3d(points, degrees)
    coeffs = method_gauss(slau_matrix)
    fig = figure()
    ...
    for elx in x:
        for ely in y:
            ...
            res_y.append(ely)
            res_z.append(elz)
    ax.plot_trisurf(res_x, res_y, res_z, color='blue', alpha=0.4)
    ax.set_xlabel('X')
    ax.set_ylabel('Y')
    ax.set_zlabel('Z')
