...
def left_border_node(k, A, B, C, D, A_, B_, C_, D_, T, f, p, F0, a0, h, i=0):
...
def right_border_node(k, A, B, C, D, A_, B_, C_, D_, T, f, p, a, a0, h, i):
    A[i] = (k[i - 1] + k[i]) / 2
...
def main():
    while True:
        ...
        a0 = 1.94 * 10 ** (-2)
        N = 100
        print("1. Task 1 - F0 = 50 (подача тепла)\n"
              "2. Task 2 - F0 = -10 (отвод тепла)\n"
              "3. Task 3 - a0 = 3*a0 (ускорение подачи тепла)\n"
              "4. Task 4 - F0 = 0 (температура стержня не меняется)\n"
              "5. Input parameters\n"
              "0. EXIT")
        test = int(input(">> "))
        if test == 0:
        ...
        while j < MAX_ITER:
            for i in range(N + 1):
                ...
                if i == 0:
                    left_border_node(k, A, B, C, D, A_, B_, C_, D_, T, f, p, F0, a0, h)
                elif i == N:
                    right_border_node(k, A, B, C, D, A_, B_, C_, D_, T, f, p, a, a0, h, N)
                else:
