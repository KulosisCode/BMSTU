...
def time_win(start_time, end_time):
    ...
    time_label = Label(win, text="Время: %-3.2f с" % (end_time - start_time), bg="lightblue", font="AvantGardeC 14", fg='black')
...
def draw_point(ev_x, ev_y, click_):
    ...
    if option_coords.get() == 0:
        ...
        canvas_win.create_oval(x - 2, y - 2, x + 2, y + 2,
                               outline='red', fill='red', activeoutline='lightblue', width=2, tag='seed_dot')
        return
    ...
    canvas_win.delete('dot')
    canvas_win.create_oval(x - 2, y - 2, x + 2, y + 2,
                           outline='grey', fill='lightblue', activeoutline='lightgreen', width=2, tag='dot')
...
def draw_axes():
    ...
    canvas_win.create_line(0, s // 2, s - 2, s // 2, fill='grey',
                           width=1, arrow=LAST, activefill='lightgreen', arrowshape="10 20 6", tag = 'coord')
    canvas_win.create_line(s // 2, s, s // 2, 2, fill='grey',
                           width=1, arrow=LAST, activefill='lightgreen', arrowshape="10 20 6", tag = 'coord')

    for i in range(0, s, s // 16):
        canvas_win.create_line(i, s / 2 - 5, i, s / 2 + 5, fill='lightblue', width=2, tag='coord')
        canvas_win.create_line(i, 0, i, s, fill='grey', width=1, dash=(1, 9), tag='coord')
        canvas_win.create_text(i, s // 2 + 20, text=f'{"%.0f" % xy_current[j]}' if i - SIZE // 2 else '',
                               fill='grey', tag='coord', font="AvantGardeC 10")

        canvas_win.create_line(s / 2 - 5, i, s / 2 + 5, i, fill='lightblue', width=2, tag='coord')
        canvas_win.create_line(0, i, s, i, fill='grey', width=1, dash=(1, 9), tag='coord')
        canvas_win.create_text(s // 2 - 20, i, text=f'{"%.0f" % xy_current[16 - j]}' if i - SIZE // 2 else '',
                               fill='grey', tag='coord', font="AvantGardeC 10")

        j += 1

    canvas_win.create_text(s - 20, s // 2 + 20, text='X', font="AvantGardeC 14", fill='grey', tag='coord')
    canvas_win.create_text(s // 2 + 20, 20, text='Y', font="AvantGardeC 14", fill='grey', tag='coord')
...
def config(event):
    if event.widget == win:
        ...
        coords_seed.place(x=3 * win_x, y=430 * win_y)
        ...
        # координаты затравки
        seed_lbl.place(x=30 * win_x, y=430 * win_y, width=110 * win_x, height=24 * win_y)
        seed_block.place(x=145 * win_x, y=430 * win_y, width=120 * win_x, height=24 * win_y)

        # добавить, изменить и удалить точку
        x_lbl.place(x=30 * win_x, y=350 * win_y, width=110 * win_x, height=18 * win_y)
        y_lbl.place(x=157 * win_x, y=350 * win_y, width=110 * win_x, height=18 * win_y)
        x_entry.place(x=30 * win_x, y=370* win_y, width=110 * win_x, height=20 * win_y)
        y_entry.place(x=157 * win_x, y=370 * win_y, width=110 * win_x, height=20 * win_y)

        add.place(x=30 * win_x, y=392 * win_y, width=237 * win_x, height=25 * win_y)

        # закраска
        answer_lbl.place(x=30 * win_x, y=500 * win_y, width=237 * win_x, height=24 * win_y)
        draw_delay.place(x=30 * win_x, y=525 * win_y)
        draw_without_delay.place(x=160 * win_x, y=525 * win_y)
...
center_lbl = Label(text="Координаты точ.", bg='lightblue', font="AvantGardeC 14", fg='black')
dots_block = Listbox(bg="#ffffff")
dots_block.configure(font="AvantGardeC 14", fg='black')

seed_lbl = Label(text="Затравка", bg='lightblue', font="AvantGardeC 14", fg='black')
...
color_lbl = Label(text="Цвет", bg='lightblue', font="AvantGardeC 14", fg='black')
answer_lbl = Label(text="Решение задачи", bg='lightblue', font="AvantGardeC 14", fg='black')
...
coords_points = Radiobutton(variable=option_coords, value=1, bg="lightgrey",
                            activebackground="lightgrey", highlightbackground="lightgrey")
coords_seed = Radiobutton(variable=option_coords, value=0, bg="lightgrey",
                          activebackground="lightgrey", highlightbackground="lightgrey")
