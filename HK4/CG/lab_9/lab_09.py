...
def draw_point(ev_x, ev_y, click_):
    ...
    if option_line.get() == 0:
        ...
        canvas_win.create_oval(x - 2, y - 2, x + 2, y + 2,
                               outline='lightblue', fill='lightblue', activeoutline='lightgreen', width=2, tag='figure_dot')
        if is_close_figure:
            click_rectangle = 0
            start_rectangle = []
...
def cut_area():
    ...
    for figure in figure_coords:
        ...
        for cur_dot_ind in range(-2, len(clipper_coords) - 2):
            line = [clipper_coords[cur_dot_ind], clipper_coords[cur_dot_ind + 1]]

            position_dot = clipper_coords[cur_dot_ind + 2]
...
# отрезок
line_lbl = Label(text="Координаты многоугольников", bg='lightblue', font="AvantGardeC 14", fg='black')
